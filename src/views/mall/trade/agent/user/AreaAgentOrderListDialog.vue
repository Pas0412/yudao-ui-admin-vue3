<template>
  <Dialog v-model="dialogVisible" title="代理订单列表" width="75%">
    <ContentWrap>
      <!-- 搜索工作栏 -->
      <el-form
        ref="queryFormRef"
...
import * as AreaAgentRecordApi from '@/api/mall/trade/agent/record'
import { BrokerageRecordBizTypeEnum } from '@/utils/constants'
import { fenToYuanFormat } from '@/utils/formatter'
import { DICT_TYPE, getIntDictOptions } from '@/utils/dict'

/** 代理订单列表 */
defineOptions({ name: 'AreaAgentOrderListDialog' })
...
const getList = async () => {
  ...
  try {
    // 处理全部的情况
    const data = await AreaAgentRecordApi.getAreaAgentRecordPage({
      ...queryParams,
