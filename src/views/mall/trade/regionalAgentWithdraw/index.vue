<template>
  <doc-alert title="【交易】地区代理提现" url="https://doc.iocoder.cn/mall/trade-brokerage/" />
  ...
  <!-- 表单弹窗：拒绝提现 -->
  <RegionalAgentWithdrawRejectForm ref="formRef" @success="getList" />
</template>
...
import { BrokerageWithdrawTypeEnum, BrokerageWithdrawStatusEnum } from '@/utils/constants'
import RegionalAgentWithdrawRejectForm from './RegionalAgentWithdrawRejectForm.vue'
...
const getList = async () => {
  loading.value = true
  try {
    const data = await RegionalAgentApi.getRegionalAgentWithdrawPage(queryParams)
...
const handleApprove = async (id: number) => {
  try {
    // 二次确认
    await ElMessageBox.confirm('确认通过该提现申请吗？')
    // 发起通过
    loading.value = true
    await RegionalAgentApi.approveRegionalAgentWithdraw(id)
...
const handleRetryTransfer = async (id: number) => {
  try {
    // 二次确认
    await ElMessageBox.confirm('确认重新转账该提现申请吗？')
    // 发起重新转账
    loading.value = true
    await RegionalAgentApi.transferRegionalAgentWithdraw(id)
