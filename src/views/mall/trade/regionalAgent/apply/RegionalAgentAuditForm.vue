  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="400px">
    ...
    >
      <el-form-item label="审核状态">
        <el-tag v-if="formData.auditStatus === AreaAgentApplyStatusEnum.APPROVED.status" type="success">
          {{ AreaAgentApplyStatusEnum.APPROVED.name }}
        </el-tag>
        <el-tag v-else-if="formData.auditStatus === AreaAgentApplyStatusEnum.REJECTED.status" type="danger">
      ...
      <el-form-item label="审核原因" prop="auditReason">
        <el-input
          ...
          :placeholder="formData.auditStatus === AreaAgentApplyStatusEnum.REJECTED.status ? '请输入拒绝原因' : '请输入审核原因'"
          :rows="3"
          type="textarea"
        />
      </el-form-item>
...
import { AreaAgentApplyStatusEnum } from '@/utils/constants'
...
/** 打开弹窗 */
const open = async (row: RegionalAgentApi.RegionalAgentVO, auditStatus: number) => {
  dialogVisible.value = true
  resetForm()
  formData.value.id = row.id
  formData.value.auditStatus = auditStatus
  
  if (auditStatus === AreaAgentApplyStatusEnum.APPROVED.status) {
...
const submitForm = async () => {
  ...
  try {
    await RegionalAgentApi.auditRegionalAgent(formData.value)
